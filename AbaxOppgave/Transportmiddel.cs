...
namespace AbaxOppgave
{
    public class Transportmiddel
    {
        ...
        protected void Add(StringBuilder text, string name, object value)
        {
            ...
            text.Append("=");
            text.Append(value);
            if (Enheter.ContainsValue(name)) text.Append(Enheter[name]);
            text.AppendLine();
        }
        ...
        {
            return transportmiddel != null && GetType() == transportmiddel.GetType() &&
                   Kjennetegn == transportmiddel.Kjennetegn;
        }
